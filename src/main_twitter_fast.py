...
from twitter_loader import TwitterDataset, TwitterLoader
...
val_loader   = TwitterLoader(dataset=val_data, phase='val')
test_data    = TwitterDataset(phase='test', wv_type=args.wv_type)
test_loader  = TwitterLoader(dataset=test_data, phase='test')
...
def val(epoch):
  ...
  val_loss = 0.
  correct = 0
  n_batch = val_loader.get_batch_num(batch_size=int(len(val_data)/100))
  for _ in range(n_batch):
    optimizer.zero_grad()
    batch = val_loader.next_batch(batch_size=int(len(val_data)/100))
    if args.cuda:
      batch['X'] = batch['X'].cuda()
      batch['Y'] = batch['Y'].cuda()
    batch['Y'] = batch['Y'].view(-1)
...
def infer():
  ...
  with open(params + '.csv', 'w') as out:
    ...
    predicts = np.array([])
    IDs = []
    n_batch = test_loader.get_batch_num(batch_size=args.test_batch_size)
    for _ in range(n_batch):
      batch = test_loader.next_batch(batch_size=args.test_batch_size)
      if args.cuda:
        batch['X'] = batch['X'].cuda()
      inputs = Variable(batch['X'])
      output = cnn_model(inputs)
      pred = np.argmax(output.data.cpu().numpy(), axis=1).flatten()
